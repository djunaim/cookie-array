const makeASnack = (position) => {
  ...
  if (position.snack.name) {
    ...
    <div class="card col-4">
      ...
      <div class="card-footer"><button class="btn btn-info buySnack ${position.snack.currentStocked < 1 ? 'disabled' : ''}" id="buy-${position.snack.id}">Purchase ${position.position}</button></div>
    </div>`;
  } else {
    domString += `
    <div class="card col-4">
